...
Product* FindProduct(string name) {
	...
	{
		string fprThing, line;
		streamoff pointer = productsFile.tellg();
...
gettingAmount:
	...
	string fileContent, lineOfFile;
	while (!file.eof()) {
		string fprThing;
		streamoff pointer = file.tellg();
		...
		if (fprThing == p->get_name()) {
			...
			string newLine = fprThing + "," + line2 + "," + to_string(stol(line3) - stol(count)) + '\n';
			fileContent += newLine;
			...
			getline(file, lineOfFile);
			fileContent += fprThing + "," + lineOfFile + '\n';
		}
	}
...
public:
	Order(Customer customer, vector<SubOrder> cart, Date_Time dt, double discount) : Cus(customer), Cart(cart), Discount(discount), ordDate(dt) {
		for (unsigned long i = 0; i < cart.size(); i++) {
		...
		string line = "(";
		for (unsigned long j = 0; j < cart.size(); j++) {
...
void SetOrder() {
	...
	if (stod(discount) < 0 || stod(discount) > 100) {
		cout << "You must be kidding!" << '\n';
		goto setDiscount;
	}
...
void ShowOrders() {
	...
	do {
		...
		if (showId == 1) {
			streamoff pointer = ordFile.tellg();
...
afterInput:
	...
	string date;
	while (!ordFile.eof()) {
		string line, bullshit, totalPrice = "0";
		if (flag2)
			break;
...
void ShowProducts() {
	...
	isEdit:
	string isEditing;
	cout << '\n' << "Do you want to edit one?(yes/no) ";
	getline(cin, isEditing);
	if (isEditing == "yes")
		EditProduct();
	...
	while (!productsFile.eof())
	{
		...
		editing:
			...
			if (editingOption == "name") {
				...
				if (stod(newStock) < 0) {
					cout << "Stock can not be negative!" << '\n';
					goto getStock;
				}
			...
			fileContent += newLine + '\n';
			while (!productsFile.eof()) {
				getline(productsFile, lineOfFile);
				if (lineOfFile == "")
					continue;
